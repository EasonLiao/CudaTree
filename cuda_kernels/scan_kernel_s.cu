...
__global__ void prefix_scan(int* sorted_labels, 
                        int *label_count,
                        int n_features, 
                        int n_samples, 
                        int stride){
  ...
  int label_offset = blockIdx.x * stride; 
  
  if(threadIdx.x == 0){
    for(int i = 0; i < MAX_NUM_LABELS; ++i)
      label_count[i + offset] = 0;

    int curr_label = sorted_labels[label_offset];
    ...
    for(int i = 1; i < n_samples; ++i) {
        ...
        curr_label = sorted_labels[label_offset + i]; 
