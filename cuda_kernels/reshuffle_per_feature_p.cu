...
__global__ void reshuffle(uint8_t* mark_table,
    ...
    shared_pos_table[threadIdx.x][1] = pos_table[thread_id * 2 + 1];

    
    for(int i = range_begin; i < range_end; ++i){
      int side = mark_table[sorted_indices[i]];
      if(side == 0){
        sorted_indices_out[shared_pos_table[threadIdx.x][0]] = sorted_indices[i];
        sorted_samples_out[shared_pos_table[threadIdx.x][0]] = sorted_samples[i];
        sorted_labels_out[shared_pos_table[threadIdx.x][0]] = sorted_labels[i];
      } 
      else{
        sorted_indices_out[shared_pos_table[threadIdx.x][1] + right_start] = sorted_indices[i];  
        sorted_samples_out[shared_pos_table[threadIdx.x][1] + right_start] = sorted_samples[i];
        sorted_labels_out[shared_pos_table[threadIdx.x][1] + right_start] = sorted_labels[i];
      }
