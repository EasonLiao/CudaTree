...
__global__ void count_total(
  ...
  uint32_t offset = blockIdx.x * MAX_NUM_LABELS * (MAX_BLOCK_PER_FEATURE + 1);
  ...
  for(IDX_DATA_TYPE i = 0; i < n_samples; i += blockDim.x){
    ...
    for(IDX_DATA_TYPE t = 0; t < stop_pos; ++t){
       if((t + i) %% n_range == 0){
        uint16_t idx = offset + (t + i) / n_range * MAX_NUM_LABELS; 
        for(uint16_t i =  threadIdx.x; i < MAX_NUM_LABELS; i += blockDim.x)
          label_total[idx + i] = shared_count[i];
       }
  ...
  uint32_t idx = int(ceil(float(n_samples)/n_range)) * MAX_NUM_LABELS;
  for(uint16_t i =  threadIdx.x; i < MAX_NUM_LABELS; i += blockDim.x)
    label_total[offset + idx + i] = shared_count[i];
}
